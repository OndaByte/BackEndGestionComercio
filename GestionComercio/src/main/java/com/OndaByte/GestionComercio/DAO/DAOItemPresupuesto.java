...
public class DAOItemPresupuesto implements DAOInterface<ItemPresupuesto> {
    
    private static Logger logger = LogManager.getLogger(DAOItemPresupuesto.class.getName());
    
    private String clave = "id";
    private GeneradorQuery<ItemPresupuesto> generadorQuery;
    
    public DAOItemPresupuesto() {
        generadorQuery = new GeneradorQuery<ItemPresupuesto>(ItemPresupuesto.class);
        generadorQuery.setClave(clave);
    }
    
    
    public Class<ItemPresupuesto> getClase() {
        return ItemPresupuesto.class;
    }
    
    public Integer alta(ItemPresupuesto t, Connection con) {
        String query;
        try {
            if(con==null)   
                con = ConexionSQL2o.getSql2o().open();
            query = "SELECT EXISTS (SELECT * FROM Presupuesto WHERE id = :presupuesto_id)";
            
            Query auxQuery = con.createQuery(query).addParameter("presupuesto_id", t.getPresupuesto_id());
            if(auxQuery.executeAndFetchFirst(Integer.class) == 0){
                logger.debug("DAOItemPresupuesto.alta() - No se encontro presupuesto asociado para dar de alta con id:" + t.getPresupuesto_id());
                return -1;
            }
    ...
    @Override
    public Integer alta(ItemPresupuesto t) {
        String query;
        Connection con = null;
        try {
            con = ConexionSQL2o.getSql2o().open();
    ...
    @Override
    public boolean modificar(ItemPresupuesto t) {
        String query;
        Connection con = null;
        try {
    ...
    @Override
    public List<ItemPresupuesto> listar() {
        String query;
        Connection con = null;
        try {
    ...
    @Override
    public List<ItemPresupuesto> listar(String... ids) {
        String query;
        Connection con = null;
        try {
    ...
    @Override
    public List<ItemPresupuesto> filtrar(List<String> campos, List<String> valores, List<Integer> condiciones, List<Boolean> conectores) {
        String query;
        Connection con = null;
        try {
    ...
    @Override
    public ItemPresupuesto buscar(String id) {
        String query;
        Connection con = null;
        try{
            query = generadorQuery.getQueryFiltarId();
            con = ConexionSQL2o.getSql2o().open();
            return (ItemPresupuesto) con.createQuery(query).addParameter("id", id).executeAndFetchFirst(this.getClase());
        } catch (Exception e) {
